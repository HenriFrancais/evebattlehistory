...
@pytest.mark.asyncio
async def test_br_char_side_roundtrip(db_session_maker) -> None:  # type: ignore[no-untyped-def]
    from app.db.models import BattleReport, BrCharSide
    ...
    async with db_session_maker() as session:
        ...
        session.add(
            BrCharSide(br_id=br_id, character_id=42, side="friendly",
                       set_by_user="fc", set_at=dt.datetime.now(dt.UTC))
        )
        await session.commit()
    ...
    async with db_session_maker() as session:
        row = (
            await session.execute(
                select(BrCharSide).where(
                    BrCharSide.br_id == br_id, BrCharSide.character_id == 42
                )
            )
        ).scalar_one()
    assert row.side == "friendly"
